...
Nesse post, vamos focar em entender sobre como esses computadores, que há anos ajudam a nossa sociedade a evoluir cada vez mais, funcionam.
...
Contudo, os computadores trabalham internamente com um sistema de contagem, denominado de **binário**, ou seja, sequências de 0s e 1s (conhecidos como *bits*), assim todo e qualquer dado precisa ser convertido para esse sistema, para que o computador consiga armazená-lo em sua memória. 

Sendo assim, a depender do arquivo que estamos manipulando, ele terá um tamanho específico, que é definido como uma cadeia desses *bits*, assim chegamos na unidade de *bytes*, onde *1 byte* corresponde uma sequência de *8 bits* e progressivamente essa cadeia vai aumentando para arquivos cada vez maiores, com os kbytes, megabytes, gigabytes...

> Em termos computacionais, a eletricidade é a vida de todo o processamento dos computadores. Essa representação dos bits em 0s e 1s, pode ser representada como desligado e ligado, e a ordem na qual esses "interruptores" estão ligados e desligados, ilustra os diferentes tipos de valores.
...
Primeiramente, embora esteja muito relacionado, o termo *algoritmo* não é exclusivamente da computação, sua definição é uma sequência finita de instruções bem definidas para resolver um problema específico. Dessa forma, receitas culinárias, manuais de montagem, e outras variadas ferramentas de instrução que tem como objetivo solucionar um problema, são algoritmos. E entenda que a palavra “problema” não tem um valor negativo; significa apenas algo que precisa chegar a um determinado resultado.
...
> Importante mencionar que a notação *Big O* leva em consideração o termo de maior expoente, visto que ele domina o crescimento da função (entenda como o algoritmo) a medida que **n** cresce, ou seja, o método de ler 1 página por vez e de ler 5 páginas no final representam o mesmo algoritmo de "Busca Linear" e possuem a mesma complexidade temporal.

### Conclusão

Por fim, releembrando os assuntos que discutimos nessa introdução a área de computação, temos:

- A base: O *bit* é a menor unidade de informação, é a base sobre o qual toda a computação é construída. Tudo que vemos, ouvimos e interagimos no mundo digital — seja uma imagem de alta definição ou uma música — é, em sua essência, uma complexa combinação de 0s e 1s.
