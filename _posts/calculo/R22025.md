...
14. $\int \frac{dx}{a^2 + x^2} = \frac{1}{a} \arctan{\frac{x}{a}} + C$
...
- Para as arco cosseno e cossecante, basta adicionar o "-".
...
$$
### Exemplo: $u = 1 + x^2 \to \frac{du}{dx} = 2x$ logo $du = 2x \ dx$
$$
...
Função algébrica ($x^2$) vem antes de trigonométrica ($\cos{x}$) para escolher como $u$.
...
$$
### Exemplo: $u = x$ | $du  = dx$ | $v = -\cos{x}$ | $dv = \sin{x} \ dx$
$$
