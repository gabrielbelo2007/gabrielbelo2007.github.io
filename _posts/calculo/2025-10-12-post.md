...
Nesse post, vamos abordar o tópico de **máximos e mínimos** de uma função, assunto muito importante como aplicação prática do conteúdo de limites e derivadas vistos até então, para **problemas de otimização** (assunto do próximo post).

Sendo assim, vamos entender sobre os extremos de uma função, e os teoremas fundamentais que auxiliam a encontrar esses pontos.
...
Dizemos que uma função pode atingir valores "extremos", ou seja, pontos de máximo (um valor maior do que os vizinhos) e pontos de mínimo (valor menor do que os vizinhos).

Dentro dessa definição de valores extremos, temos os valores extremos locais e os globais: 

- **Local**: O menor ou maior valor em relação aos pontos no seu entorno.
- **Global**: O menor ou maior valor absoluto no domínio da função.

Exemplificando, existem funções que crescem ou decrescem indefinidamente, nesse caso não se tem um valor extremo absoluto, porém pode haver um ponto extremo dentro de uma região, e portanto ele seria um ponto de extremo local.

Para o caso de funções que não crescem ou decrescem indefinidamente, o ponto de extremo local tende a coincidir com o ponto de extremo global. 

> Em funções períodicas esses extremos, embora "globais", se repetem ao longo do domínio.

Em termos matemáticos, uma função tem máximo global, se para ponto $$c$$ do $$Dom(f)$$, o valor de $$f(c)$$ seja o maior para qualquer outro $$x$$ pertencente ao domínio (raciocínio analógo para o mínimo global).
...
A importância disso, está na resolução de **problemas de otimização**, onde dado uma função com intervalo fechado, mesmo que não saibamos a príncipio quais são seus extremos. 

Temos a garantia de que existem dois pontos percentences a esse intervalo, que são o máximo e mínimo global dessa função.

Para verificar quais são esses pontos, temos:

- Encontrar os valores de $$f$$ nos pontos críticos de $$f$$ para $$x \in (a, b)$$
- Encontar os valores de $$f$$ nos extremos do intervalo $$f(a)$$ e $$f(b)$$
- O maior valor da dentre todos os encontrados é o máximo global, e o menor valor é o mínimo global.
...
Esse teorema, generaliza a ideia apresentada no **Teorema de Rolle**, e isso traz algumas consequências importantes, para o estudo das funções.
